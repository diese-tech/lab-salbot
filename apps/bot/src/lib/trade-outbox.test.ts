...
vi.mock('@salbot/db', () => ({
  ...
  getCanonicalOrganizationRoleStates: vi.fn(),
...
describe('roster trade outbox projections', () => {
  it('reconciles the durable proposal marker after restart and persists the discovered message ID', async () => {
    vi.mocked(getRosterTrade).mockResolvedValue({
      ...completedTrade(), status: 'awaiting_acceptance', proposalMessageId: null,
    });
    const edit = vi.fn().mockResolvedValue(undefined);
    const existing = {
      id: 'proposal-existing', content: '', edit,
      embeds: [new EmbedBuilder().setFooter({ text: 'sal-trade:trade-1' }).toJSON()],
    };
    const send = vi.fn();
    const client = { channels: { fetch: vi.fn().mockResolvedValue(textChannel(
      new Collection([['proposal-existing', existing]]), send,
    )) } };

    await expect(createOutboxProjector(client as never, {} as never)(outbox('discord_trade_proposal_projection')))
      .resolves.toBe('proposal-existing');
    expect(edit).toHaveBeenCalledOnce();
    expect(send).not.toHaveBeenCalled();
    expect(updateRosterTradeProposalMessage).toHaveBeenCalledWith({}, 'trade-1', 'proposal-existing');
  });

  it('projects accepted trades through the shared admin-review controls', async () => {
    vi.mocked(getRosterTrade).mockResolvedValue({ ...completedTrade(), status: 'awaiting_admin' });
    vi.mocked(getPendingAction).mockResolvedValue({
      id: 'action-1', type: 'roster_trade', status: 'pending', requested_by_discord_id: 'captain-1',
      match_id: null, division_id: 'solar', payload_json: {
        transactionId: 'trade-1', revision: 1, source: 'discord_workflow',
      },
      admin_note: null, admin_review_message_id: null, public_receipt_message_id: null,
      approved_by_discord_id: null, approved_at: null,
    } as never);
    const send = vi.fn().mockResolvedValue({ id: 'admin-review-1' });
    const client = { channels: { fetch: vi.fn().mockResolvedValue(textChannel(new Collection(), send)) } };

    await expect(createOutboxProjector(client as never, {} as never)(outbox('discord_trade_admin_review')))
      .resolves.toBe('admin-review-1');
    expect(send.mock.calls[0][0].components[0].components).toHaveLength(3);
    expect(updateRosterTradeAdminReviewMessage).toHaveBeenCalledWith({}, 'action-1', 'admin-review-1');
  });
  ...
  it('publishes a completed uneven trade only to CHANNEL_TRANSACTIONS', async () => {
    ...
    await expect(createOutboxProjector(client as never, {} as never)(outbox('discord_transaction_bulletin')))
    ...
    expect(embed.description).toBe('[SOLAR] FF traded Crow + Kestrel to TC for The_Expert133');
  ...
  it('reuses a bulletin discovered by its stable marker instead of reposting', async () => {
    const existing = {
      id: 'existing-bulletin', content: '',
      embeds: [new EmbedBuilder().setFooter({ text: 'sal-operation:trade-1' }).toJSON()],
    };
    const send = vi.fn();
    const client = { channels: { fetch: vi.fn().mockResolvedValue(textChannel(
      new Collection([['existing-bulletin', existing]]), send,
    )) } };
  ...
  it('pauses an ambiguous bulletin send and creates a stable private reconciliation alert', async () => {
    const transactionChannel = textChannel(
      new Collection(), vi.fn().mockRejectedValue(new Error('request timed out')),
    );
    const alertSend = vi.fn().mockResolvedValue({ id: 'delivery-alert-1' });
    const adminChannel = textChannel(new Collection(), alertSend);
    const client = { channels: { fetch: vi.fn().mockImplementation((id: string) =>
      Promise.resolve(id === 'transactions' ? transactionChannel : adminChannel)) } };
  ...
  it('reconciles only configured organization roles from canonical roster state', async () => {
    vi.mocked(getCanonicalOrganizationRoleStates).mockResolvedValue([{
      playerId: 'p1', discordId: 'member-1', playerName: 'Crow', orgId: 'tc',
      desiredOrganizationRoleId: 'role-tc', knownOrganizationRoleIds: ['role-ff', 'role-tc'],
    }]);
    const remove = vi.fn().mockResolvedValue(undefined);
    const add = vi.fn().mockResolvedValue(undefined);
    const member = { roles: { cache: new Collection([
      ['role-ff', {}], ['unrelated-role', {}],
    ]), remove, add } };
    ...
    await createOutboxProjector(client as never, {} as never)(outbox('discord_organization_role_reconciliation'));
  ...
  it('alerts admins and leaves role work retryable when Discord role sync fails', async () => {
    vi.mocked(getCanonicalOrganizationRoleStates).mockResolvedValue([{
      playerId: 'p1', discordId: null, playerName: 'Crow', orgId: 'tc',
      desiredOrganizationRoleId: 'role-tc', knownOrganizationRoleIds: ['role-ff', 'role-tc'],
    }]);
    ...
    await expect(createOutboxProjector(client as never, {} as never)(outbox('discord_organization_role_reconciliation')))
      .rejects.toThrow('reconciliation incomplete');
    const description = alertSend.mock.calls[0][0].embeds[0].toJSON().description;
...
function completedTrade() {
  return {
    ...
    status: 'completed', currentRevision: 1, proposerOrgId: 'ff', receiverOrgId: 'tc',
    pendingActionId: 'action-1', proposalChannelId: 'trade-channel', proposalMessageId: 'proposal-1',
    proposer: { id: 'ff', name: 'Food Fighters', tag: 'FF' },
    receiver: { id: 'tc', name: 'The Crew', tag: 'TC' },
    movements: [
      { id: 'p1', name: 'Crow', discordId: 'd1', fromOrgId: 'ff', toOrgId: 'tc' },
      { id: 'p2', name: 'Kestrel', discordId: 'd2', fromOrgId: 'ff', toOrgId: 'tc' },
      { id: 'p3', name: 'The_Expert133', discordId: 'd3', fromOrgId: 'tc', toOrgId: 'ff' },
    ],
  };
}
...
function outbox(topic: string) {
  return {
    id: 'outbox-1', topic, aggregate_type: 'roster_transaction', aggregate_id: 'trade-1',
    event_type: 'roster_trade_completed', deduplication_key: `trade-1:${topic}`,
    payload: topic === 'discord_organization_role_reconciliation'
      ? { seasonId: 'season-1', playerIds: ['p1'] }
      : topic === 'discord_trade_proposal_projection'
        ? { transactionId: 'trade-1', revision: 1, channelId: 'trade-channel' }
        : { operationId: 'trade-1' },
    state: 'processing', attempts: 1, available_at: '2026-08-22T00:00:00Z',
    lease_owner: 'worker', lease_expires_at: '2026-08-22T00:01:00Z', last_error: null,
    external_id: null, completed_at: null, created_at: '2026-08-22T00:00:00Z', updated_at: '2026-08-22T00:00:00Z',
  };
}

function textChannel(messages: Collection<string, unknown>, send: ReturnType<typeof vi.fn>) {
  return {
    isTextBased: () => true,
