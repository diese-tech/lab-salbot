...
import { hasAdminCommandAccess } from "../lib/command-access";
...
export async function execute(interaction: ChatInputCommandInteraction) {
  const isAdmin = hasAdminCommandAccess(interaction.member);
  const captain = isAdmin
    ? null
    : await getCaptainByDiscordId(db, interaction.user.id);
  if (!isAdmin && !captain) {
    await interaction.reply({
      content: "You are not registered as a captain.",
      ephemeral: true,
    });
    return;
  }
  ...
  const matches = isAdmin
    ? await getEligibleMatchesForOperator(db)
    : await getEligibleMatchesForCaptain(
        db,
...
export async function handleRescheduleModal(
  ...
  const reason =
    interaction.fields.getTextInputValue("reason").trim() || undefined;
  ...
  const homeOrg = match.home_org as unknown as {
    id: string;
    name: string;
    tag: string;
  };
  const awayOrg = match.away_org as unknown as {
    id: string;
    name: string;
    tag: string;
  };
  ...
  const receiptEmbed = buildRescheduleReceiptEmbed(
    matchInfo,
    newDate,
    newTime,
    interaction.user.id,
  ...
  const adminEmbed = buildRescheduleAdminEmbed(
    matchInfo,
    newDate,
    newTime,
    reason,
  ...
  await interaction.editReply(
    "✅ Reschedule request submitted for admin review.",
  );
}
