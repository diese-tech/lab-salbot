...
import {
  execute,
  handleTradeActionButton,
  handleTradeSelect,
  handleTradeWizardButton,
} from "./trade";
...
describe("/trade entry authorization", () => {
      ...
      organizations: [
        {
          ...
          name: "Alpha",
          tag: "ALP",
          organizationRoleId: ORG_A_ROLE_ID,
          captainDiscordIds: [],
          players: [{ id: "a1", name: "Alpha One", discordId: null }],
        },
        {
          id: "org-b",
          name: "Beta",
          tag: "BET",
          organizationRoleId: ORG_B_ROLE_ID,
          captainDiscordIds: ["captain-2"],
          players: [{ id: "b1", name: "Beta One", discordId: "b1-discord" }],
        },
      ],
    });
    const reply = vi.fn();
  ...
  it("rejects a Captain-role holder missing canonical roster captain identity", async () => {
    vi.mocked(getRosterTradeSetup).mockResolvedValue({
      seasonId: "season-1",
      divisionId: "solar",
      tradesOpen: true,
    ...
    await execute({
      ...
      guildId: "guild-1",
      user: { id: "captain-1" },
      member: { roles: [OPERATOR_ROLE_ID, CAPTAIN_ROLE_ID] },
      reply,
    } as never);
    ...
    expect(reply).toHaveBeenCalledWith(
      expect.objectContaining({
        content: expect.stringContaining(
          "not an admin, organization owner/advisor, or current captain",
        ),
        ephemeral: true,
      }),
    );
  ...
  it("keeps selections private until Post Proposal and supports an uneven exchange", async () => {
    vi.mocked(getRosterTradeSetup).mockResolvedValue({
      ...
      organizations: [
        ...
        },
        {
          ...
          organizationRoleId: ORG_B_ROLE_ID,
          captainDiscordIds: ["captain-2"],
          players: [{ id: "b1", name: "Beta One", discordId: null }],
        },
      ],
    });
    ...
    expect(reviewStep.content).toContain(
    ...
    expect(createRosterTrade).toHaveBeenCalledWith(
      {},
      expect.objectContaining({
        offeredPlayerIds: ["a1", "a2"],
        requestedPlayerIds: ["b1"],
        proposalChannelId: "channel-solar",
      }),
    );
  ...
  it("re-reads durable state and rejects an unauthorized public-card interaction", async () => {
    ...
    await expect(
      handleTradeActionButton({
        customId: "trade:accept:trade-1:1",
        channelId: "channel-solar",
        user: { id: "spectator" },
        member: { roles: [] },
        deferReply: vi.fn(),
      ...
      "Only an authorized representative of the receiving organization",
    );
  ...
  it("binds receiving-captain acceptance to the card revision and leaves execution to admin review", async () => {
    ...
    expect(acceptRosterTrade).toHaveBeenCalledWith(
      {},
      {
        transactionId: "trade-1",
        expectedRevision: 1,
        actorDiscordId: "temporary-captain-2",
      },
    );
    expect(editReply).toHaveBeenCalledWith(
      expect.stringContaining("no roster changed yet"),
    );
  });
  ...
  it("rejects stale card controls before calling a mutation RPC", async () => {
    ...
    ).rejects.toThrow("card is stale");
    expect(acceptRosterTrade).not.toHaveBeenCalled();
  });
});

function componentCustomId(payload: {
