...
export const data = {
  name: "trade",
  description:
    "Propose a player trade to another organization in your division.",
} as const;
...
export async function execute(
  ...
  if (!divisionId) {
    await interaction.reply({
      content:
        "Use `/trade` in your division’s configured trade-block channel.",
      ephemeral: true,
    });
    return;
  }
  if (!interaction.guildId) {
    await interaction.reply({
      content: "`/trade` is only available in the SAL server.",
      ephemeral: true,
    });
    return;
  }
  ...
  if (!setup || !setup.tradesOpen) {
    await interaction.reply({
      content: "Trades are not currently open for this division.",
      ephemeral: true,
    });
    return;
  }
  ...
  const select = new StringSelectMenuBuilder()
    ...
    .addOptions(
      authorizedOrganizations.slice(0, 25).map((org) => ({
        label: `${org.name} (${org.tag})`,
        value: org.id,
      })),
    );
  await interaction.reply({
    content:
      "**Trade Proposal — Step 1 of 5:** Select the organization you represent.",
    components: [
...
export async function handleTradeSelect(
  ...
  if (action === "tr_org") {
    ...
    await interaction.update({
      content:
        "**Trade Proposal — Step 2 of 5:** Select one or more players you are offering.",
  ...
  if (action === "tr_offer") {
    ...
    const options = wizard.organizations.filter(
      (org) => org.id !== proposer.id,
    );
    if (options.length === 0)
      throw new UserFacingError(
        "No other active organization is available in this division.",
      );
    await interaction.update({
      content:
        "**Trade Proposal — Step 3 of 5:** Select the other organization.",
  ...
  if (action === "tr_recv") {
    ...
    if (receiver.id === wizard.proposerOrgId)
      throw new UserFacingError(
        "A trade requires two different organizations.",
      );
    wizard.receiverOrgId = receiver.id;
    wizard.requestedPlayerIds = undefined;
    await interaction.update({
      content:
        "**Trade Proposal — Step 4 of 5:** Select one or more players you are requesting.",
      components: [
        playerSelectRow(`tr_request:${wizard.id}`, receiver.players),
      ],
    });
    return;
  }
    ...
    await interaction.update({
      ...
      components: [
        new ActionRowBuilder<ButtonBuilder>().addComponents(
          new ButtonBuilder()
            .setCustomId(`tr_post:${wizard.id}`)
            .setLabel(wizard.transactionId ? "Post Counter" : "Post Proposal")
            .setStyle(ButtonStyle.Primary),
          new ButtonBuilder()
            .setCustomId(`tr_cancel:${wizard.id}`)
            .setLabel("Cancel")
...
export async function handleTradeWizardButton(
  ...
  if (action === "tr_cancel") {
    ...
    await interaction.update({
      content: "Trade draft cancelled. Nothing was posted.",
      components: [],
    });
    return;
  }
  ...
  if (!wizard.offeredPlayerIds?.length || !wizard.requestedPlayerIds?.length) {
    throw new UserFacingError(
      "This trade draft is incomplete. Start `/trade` again.",
    );
  }
  ...
  const result =
    wizard.transactionId && wizard.expectedRevision
...
async function requireCurrentWizardAuthorization(
  ...
  if (
    ...
    throw new UserFacingError(
      "Trade availability or role configuration changed. Start `/trade` again.",
    );
  }
  const proposer = setup.organizations.find(
    (org) => org.id === wizard.proposerOrgId,
  );
  const receiver = setup.organizations.find(
    (org) => org.id === wizard.receiverOrgId,
  );
  if (
    !proposer ||
    !receiver ||
    ...
    throw new UserFacingError(
      "Your current captain, owner/advisor, or admin authorization no longer permits this proposal.",
    );
  }
  ...
  if (wizard.transactionId) {
    ...
    if (
      !trade ||
      trade.currentRevision !== wizard.expectedRevision ||
      trade.receiverOrgId !== proposer.id ||
      trade.proposerOrgId !== receiver.id
    ) {
      throw new UserFacingError(
        "This trade changed while you prepared the counteroffer. Use the current proposal.",
      );
    }
  }
...
export async function handleTradeActionButton(
  interaction: ButtonInteraction,
): Promise<void> {
  const [, action, transactionId, revisionRaw] =
  ...
  if (trade.currentRevision !== expectedRevision) {
    throw new UserFacingError(
      "This trade card is stale. Use the current proposal revision.",
    );
  }
  const setup = await getRosterTradeSetup(db, trade.divisionId);
  if (!setup || setup.seasonId !== trade.seasonId || !setup.captainRoleId) {
    throw new UserFacingError(
      "Current trade authorization cannot be resolved. Ask an admin to review configuration.",
    );
  }
  const captainRoleId = setup.captainRoleId;
  ...
  if (action === "counter") {
    requireActorOrganization(
      actorOrgIds,
      trade.receiverOrgId,
      "Only an authorized representative of the receiving organization can counter.",
    );
  ...
  if (action === "accept") {
    requireActorOrganization(
      actorOrgIds,
      trade.receiverOrgId,
      "Only an authorized representative of the receiving organization can accept.",
    );
    await acceptRosterTrade(db, {
      transactionId,
      expectedRevision,
      actorDiscordId: interaction.user.id,
    });
    await interaction.editReply(
      "Accepted. The trade is now waiting for administrator review; no roster changed yet.",
    );
  } else if (action === "decline") {
    requireActorOrganization(
      actorOrgIds,
      trade.receiverOrgId,
      "Only an authorized representative of the receiving organization can decline.",
    );
    await declineRosterTrade(db, {
      transactionId,
      expectedRevision,
      actorDiscordId: interaction.user.id,
    });
    ...
    requireActorOrganization(
      actorOrgIds,
      trade.proposerOrgId,
      "Only an authorized representative of the proposing organization can withdraw.",
    );
    await cancelRosterTrade(db, {
      transactionId,
      expectedRevision,
      actorDiscordId: interaction.user.id,
      mode: "withdraw",
    ...
    if (
      !actorOrgIds.includes(trade.proposerOrgId) &&
      !actorOrgIds.includes(trade.receiverOrgId)
    ) {
      throw new UserFacingError(
        "Only an authorized representative of a participating organization can revoke consent.",
      );
    }
    await cancelRosterTrade(db, {
      transactionId,
    ...
    await interaction.editReply(
      "Consent revoked before administrator execution.",
    );
  } else {
...
async function startCounterWizard(
  ...
  if (!proposer || !receiver)
    throw new UserFacingError(
      "The current trade organizations are unavailable.",
    );
  const wizardId = randomUUID().slice(0, 12);
  const offered = trade.movements
    .filter((move) => move.fromOrgId === proposer.id)
    .map((move) => move.id);
  tradeWizards.set(wizardId, {
    id: wizardId,
    actorDiscordId: interaction.user.id,
    ...
    expectedRevision: trade.currentRevision,
    expiresAt: Date.now() + 15 * 60_000,
  });
  await interaction.reply({
...
export async function handleTradeCounterSelect(
  interaction: StringSelectMenuInteraction,
): Promise<void> {
  const [action, wizardId] = interaction.customId.split(":");
  const wizard = requireWizard(
  ...
  if (action === "tr_counter_offer") {
    ...
    const requested = wizard.transactionId
      ? ((await getRosterTrade(db, wizard.transactionId))?.movements
          .filter((move) => move.fromOrgId === receiver.id)
          .map((move) => move.id) ?? [])
      : [];
    await interaction.update({
      content: `**Counteroffer — Step 2 of 3:** Update players requested from ${receiver.tag}.`,
      components: [
        playerSelectRow(`tr_request:${wizard.id}`, receiver.players, requested),
      ],
    });
  }
}
...
function requireWizard(
  ...
  if (
    ...
    throw new UserFacingError(
      "This private trade wizard expired or belongs to another user. Start `/trade` again.",
    );
  }
  return wizard;
}
...
function pruneWizards(): void {
  const now = Date.now();
  for (const [id, wizard] of tradeWizards)
    if (wizard.expiresAt <= now) tradeWizards.delete(id);
}
...
function assertSelectedPlayers(
  ...
  if (
    ...
    throw new UserFacingError(
      "One or more selected players are no longer on that roster.",
    );
  }
}

function organizationSelectRow(
  customId: string,
  organizations: RosterTradeOrganization[],
) {
...
function playerSelectRow(
  customId: string,
  players: RosterTradeOrganization["players"],
  defaults: string[] = [],
) {
  if (players.length === 0)
    throw new UserFacingError(
      "That organization has no current rostered players.",
    );
  return new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
    new StringSelectMenuBuilder()
      .setCustomId(customId)
      .setPlaceholder("Select one or more players")
      .setMinValues(1)
      ...
      .addOptions(
        players.slice(0, 25).map((player) => ({
          label: player.name,
          value: player.id,
          default: defaults.includes(player.id),
        })),
      ),
  );
}
...
function tradeReviewText(
  ...
  const names = (org: RosterTradeOrganization, ids: string[]) =>
    ids
      .map((id) => org.players.find((player) => player.id === id)?.name ?? id)
      .join(", ");
  return [
...
function requireActorOrganization(
  actorOrgIds: string[],
