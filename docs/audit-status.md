...
clean teardown, or a live Discord flow.
...
| `SAL-DB-01` | SALbot verifies immutable `db-v1.5.0` at commit `80948170d7855ecb9e3f7a93f373f91713c937ba`, vendors the generated types, and validates database JSON at runtime. | [#41](https://github.com/diese-tech/lab-salbot/issues/41) |
...
- `SAL-DB-01`: [`diese-tech/sal-database`](https://github.com/diese-tech/sal-database) is the sole owner of active Supabase migrations, generated types, releases, drift detection, and production pushes. SALbot pins `db-v1.5.0` through `db-contract.lock.json`.
- `SAL-OPS-02`: the transactional RPC and lease contract shipped in the
  protected `db-v1.5.0` release. SALBot owns projection delivery and deployment
  verification through [#42](https://github.com/diese-tech/lab-salbot/issues/42)
  and [#45](https://github.com/diese-tech/lab-salbot/issues/45).
...
A finding closes only when its issue links the merged PR and records the relevant
verification evidence. The remaining production-readiness finding (#45) stays
open until Railway health, shutdown, singleton, and one live decision/projection
flow are recorded in the private remediation ledger.
