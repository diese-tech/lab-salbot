...
export type RosterTradeOrganization = {
  ...
  name: string;
  tag: string;
  /** Legacy organization-wide owner/advisor authority role; never assigned to players. */
...
function mutationResult(value: unknown): RosterTradeMutationResult {
  ...
  return {
    ...
    ...(typeof row.pendingActionId === "string"
      ? { pendingActionId: row.pendingActionId }
      : {}),
    ...(Array.isArray(row.outboxIds)
      ? {
          outboxIds: row.outboxIds.filter(
...
export async function getRosterTradeSetup(
  ...
  const [
    ...
    { data: captainMapping, error: captainError },
    { data: seasonOrgs, error: orgError },
    { data: rosters, error: rosterError },
    { data: organizationRoles, error: organizationRolesError },
  ] = await Promise.all([
    client
      .from("season_transaction_settings")
      .select("trades_open,drops_open")
      .eq("season_id", season.id)
      .eq("division_id", divisionId)
      .maybeSingle(),
    client
      .from("captain_role_mappings")
    ...
    client
      .from("season_rosters")
      .select(
        "player_id,org_id,is_captain,player:players!inner(discord_id,ign,display_alias)",
      )
      .eq("season_id", season.id)
      .eq("division_id", divisionId)
  ...
  const organizations = (
    (seasonOrgs ?? []) as Array<Record<string, unknown>>
  ).map((row) => {
  ...
  return {
    ...
    dropsOpen: settings?.drops_open === true,
    captainRoleId:
      typeof captainMapping?.discord_role_id === "string"
        ? captainMapping.discord_role_id
        : null,
    organizations,
  };
}
...
export async function getRosterTrade(
  db: SupabaseClient,
  transactionId: string,
): Promise<RosterTrade | null> {
  const client = db;
  ...
  if (
    ...
    typeof trade.receiver_org_id !== "string"
  )
    return null;
  const receiverOrgId = trade.receiver_org_id;
  const [
    ...
    client
      .from("orgs")
      .select("id,name,tag")
      .in("id", [trade.proposer_org_id, receiverOrgId]),
    client
      .from("roster_transaction_movements")
      .select(
        "player_id,from_org_id,to_org_id,player:players!inner(ign,display_alias,discord_id)",
      )
      .eq("transaction_id", transactionId)
      .eq("revision", trade.current_revision),
  ]);
  ...
  const receiver = orgRows.find((row) => row.id === receiverOrgId);
  if (!proposer || !receiver)
    throw new Error(`Roster trade ${transactionId} has missing organizations.`);
  ...
  return {
    ...
    seasonId: String(trade.season_id),
    divisionId: String(trade.division_id),
    status: String(trade.status),
    currentRevision: Number(trade.current_revision),
    proposerOrgId: String(trade.proposer_org_id),
    receiverOrgId,
    pendingActionId: String(trade.pending_action_id),
    proposalChannelId:
      typeof trade.proposal_channel_id === "string"
        ? trade.proposal_channel_id
        : null,
    proposalMessageId:
      typeof trade.proposal_message_id === "string"
        ? trade.proposal_message_id
        : null,
    proposer: toOrg(proposer),
    receiver: toOrg(receiver),
    movements: ((movements ?? []) as Array<Record<string, unknown>>).map(
      (movement) => {
        ...
        return {
          ...
          discordId:
            typeof player.discord_id === "string" ? player.discord_id : null,
          fromOrgId: String(movement.from_org_id),
          toOrgId: stringValue(movement, "to_org_id"),
        };
      },
    ),
  };
...
export async function updateRosterTradeAdminReviewMessage(
  ...
  const { error } = await db
    ...
    .update({
      admin_review_message_id: messageId,
      updated_at: new Date().toISOString(),
    })
...
export type CanonicalTeamRoleState = {
  playerId: string;
  discordId: string | null;
  playerName: string;
  desiredTeamRoleId: string | null;
  knownDivisionTeamRoleIds: string[];
  orgId: string | null;
};

export async function getCanonicalTeamRoleStates(
  db: SupabaseClient,
  seasonId: string,
  divisionId: string,
  playerIds: string[],
): Promise<CanonicalTeamRoleState[]> {
  const client = db;
  const [
    ...
    client
      .from("season_organization_role_mappings")
      .select("org_id,discord_role_id")
      .eq("season_id", seasonId)
      .eq("division_id", divisionId),
    client
      .from("season_rosters")
      .select(
        "player_id,org_id,roster_status,player:players!inner(discord_id,ign,display_alias)",
      )
      .eq("season_id", seasonId)
      .eq("division_id", divisionId)
  ...
  const roleByOrg = new Map(
    mappingRows.map((row) => [String(row.org_id), String(row.discord_role_id)]),
  );
  const knownDivisionTeamRoleIds = [...roleByOrg.values()];
  return ((rosters ?? []) as Array<Record<string, unknown>>).map((roster) => {
    const player = record(roster.player, "canonical roster player");
    const orgId =
      roster.roster_status === "active" && typeof roster.org_id === "string"
        ? roster.org_id
        : null;
    return {
      ...
      playerName:
        ...
        player.display_alias.length > 0
          ? player.display_alias
          : stringValue(player, "ign"),
      desiredTeamRoleId: orgId ? (roleByOrg.get(orgId) ?? null) : null,
      knownDivisionTeamRoleIds,
      orgId,
    };
  });
}
